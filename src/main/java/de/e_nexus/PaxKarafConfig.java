...
public class PaxKarafConfig implements FrameworkConfigContributor {
    ...
    public void contributeKarafConfiguration(final Properties config,
        ...
        paxurl.setProperty(PREFIX + "defaultRepositories", join(",",
                "${karaf.home.uri}${karaf.default.repository}"
                        + "@id=system.repository@snapshots",
                "${karaf.data.uri}kar@id=kar.repository@multi@snapshots",
                "${karaf.base.uri}${karaf.default.repository}"
                        + "@id=child.system.repository@snapshots"));
        paxurl.setProperty(PREFIX + "socket.tcpNoDelay", "true");
        paxurl.setProperty(PREFIX + "certificateCheck", "true");
        paxurl.setProperty(PREFIX + "repositories", join(", ",
                "https://repo1.maven.org/maven2@id=central",
                "https://repository.apache.org/content/groups/"
                        + "snapshots-group@id=apache@snapshots@noreleases",
                "https://oss.sonatype.org/content/repositories"
                        + "/ops4j-snapshots@"
                        + "id=ops4j.sonatype.snapshots.deploy@"
                        + "snapshots@noreleases"));
